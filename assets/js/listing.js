...
function generateQueryListing() {
  ...
  queryListing.forEach((listing) => {
    ...
    indivListing.classList.add('col-lg-4', 'col-md-6', 'item', 'loaded-element');
          ...
          <div class="pi-agent">
            <div class="pa-item">
              <div class="pa-info">
                ...
                <h6><a href="profile.html?id=${listing.ownerID}">${listing.owner}</a></h6>
              </div>
            </div>
          </div>
        </div>
...
function filterListings() {
  ...
  let filters = new searchFilterObject(JSON.parse(sessionStorage.getItem('searchFilter')));
  console.log(sessionStorage.getItem('searchFilter'));
  //Filter the listing based on the filters
  let filteredListings = listings.filter(listing =>
    (filters.location ? listing.location.toLowerCase().includes(filters.location.toLowerCase()) : true) &&
    (filters.searchType ? listing.name.toLowerCase().includes(filters.searchType.toLowerCase()) : true) &&
...
function sortListing(sortType) {
  ...
  switch (sortType) {
    case 'rating-low':
      listingList.sort((a, b) => a.listing.reviewScore - b.listing.reviewScore);
      break;
    case 'rating-high':
      listingList.sort((a, b) => b.listing.reviewScore - a.listing.reviewScore);
      break;
    case 'price-low':
